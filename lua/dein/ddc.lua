...
ddc.patch_global('sourceOptions',{
        _ = {
                ...
                converters = {'converer_fuzzy','converter_remove_overlap'};
        };
        around = {
                mark = 'A';
        };
...
function _G.ddc_commandline_pre ()
        ...
        fn([[ddc#custom#patch_buffer]])('sources',{'necovim','cmdline_history','around'});
...
_G.ddc_skkeleton_post = ddc_commandline_post;
fn('autocmd! User skkeleton-enable-pre call v:lua.ddc_skkeleton_pre()');
fn('autocmd! User skkeleton-disenable-pre call v:lua.ddc_skkeleton_post()');


api.nvim_set_keymap('i','<TAB>',[[pum#visible() ? pum#map#insert_relative(+1) : (col('.') <= 1 <Bar><Bar> getline('.')[col('.') - 2] =~# '\s') ? '<TAB>' ? ddc#manual_complete()]],{noremap = true,expr = true});
api.nvim_set_keymap('!','<S-TAB>','<Cmd>call pum#map#insert_relative(-1)<CR>',{noremap = true});
api.nvim_set_keymap('c','<TAB>',[[pum#visible() ? pum#map#insert_relative(+1) : ddc#manual_complete()]],{noremap = true,expr = true});
